...
export const governmentNameGenerator: SiteNameGenerator = {
  generateName(fragments: GeneratorSiteFragmentPlain[], filters: GenerateSiteNameOptions): string {
    ...
    // Step 2: Apply `functionType` filtering via helper (handles data fallback)
    const functionTypes =
        filters.functionType ??
        (filters.data?.type === "government" && Array.isArray(filters.data.function)
            ? filters.data.function
            : undefined);
    filtered = filterBySubType(filtered, "functionType", functionTypes);
