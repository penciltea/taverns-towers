...
type NormalizedGuildInput = SiteFormData & {
  size: string;
  terrain: string[];
  tags: string[];
  climate: string;
...
function normalizeInput(data: SiteGenerationInput): NormalizedGuildInput {
  const isGuild = data.type === "guild";
  ...
  return {
    ...
    guildType:
      isGuild && data.guildType && data.guildType !== "" 
        ? data.guildType
        : "random",
  };
}
